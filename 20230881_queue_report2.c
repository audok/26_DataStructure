...
bool cancelJob(PrintQueue* q, char* documentName) {
    ...
    int i = q->front;
    while (i != q->rear) {
        if (!strcmp(q->jobList[i].documentName, documentName)) {
            cancelIndex = i;
            break;
        }
        i = (i + 1) % SIZE;
    ...
    if (cancelIndex == -1) {
        ...
        while (cancelIndex != (q->rear - 1 + SIZE) % SIZE) {
            q->jobList[cancelIndex] = q->jobList[(cancelIndex + 1) % SIZE];
            cancelIndex = (cancelIndex + 1) % SIZE;
        }
...
int main() {
    ...
    while (true) {
        ...
        case 3:
            ...
            break;
        case 4:
            printf("취소할 문서 이름을 입력해주세요: ");
            scanf("%s", documentName);
            result = cancelJob(&q, documentName);
            if (result) {
                printf("'%s' 문서가 대기열에서 취소되었습니다.\n", documentName);
            } else {
                printf("해당 문서가 없습니다.\n");
			}
            break;
